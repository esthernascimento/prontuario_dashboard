...
class EnfermeiroController extends Controller
{
    ...
    {
        $enfermeiro = Enfermeiro::with('usuario')->get();
        return view('admin.manutencaoEnfermeiro', compact('enfermeiro'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            'nomeEnfermeiro' => 'required|string|max:255',
            'emailEnfermeiro' => 'required|email|unique:tbEnfermeiro,emailEnfermeiro,' . $id . ',idEnfermeiroPK',
            'corenEnfermeiro' => 'required|string|max:50',
            'especialidadeEnfermeiro' => 'nullable|string|max:100',
            'genero' => 'required|string|max:20',
            'nomeUsuario' => 'required|string|max:255',
            'emailUsuario' => 'required|email|unique:tbUsuario,emailUsuario,' . $enfermeiro->usuario->idUsuarioPK . ',idUsuarioPK',
        ]);
    ...
    {
        $enfermeiro = Enfermeiro::with('usuario')->findOrFail($id);
        return view('enfermeiros.visualizarEnfermeiro', compact('enfermeiro'));
    }
